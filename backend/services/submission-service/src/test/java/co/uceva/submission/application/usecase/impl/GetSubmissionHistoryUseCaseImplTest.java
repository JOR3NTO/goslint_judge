...
class GetSubmissionHistoryUseCaseImplTest {

    @Mock
    private SubmissionRepository submissionRepository;

    @InjectMocks
    private GetSubmissionHistoryUseCaseImpl useCase;
    ...
    void shouldReturnHistory() {
        GetSubmissionHistoryCommand command = new GetSubmissionHistoryCommand(problemId, teamId);
        List<Submission> submissions = List.of(
                SubmissionFixtures.aSubmission(UUID.randomUUID()),
                SubmissionFixtures.aSubmission(UUID.randomUUID())
        );
        when(submissionRepository.findByProblemIdAndTeamId(problemId, teamId)).thenReturn(submissions);

        List<Submission> result = useCase.execute(command);
